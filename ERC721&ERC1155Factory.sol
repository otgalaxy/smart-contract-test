...
contract TokenFactory {
    ...
    // Mapping of token IDs to deployed token addresses
    mapping(uint256 => address) public tokenIdToAddress;

    // Mapping of token IDs to their token type (ERC721, ERC1155)
    mapping(uint256 => TokenType) public tokenIdToType;

    // Mapping to track tokens deployed by each user, categorized by token type
    mapping(address => mapping(TokenType => address[])) private userDeployedTokens;
    ...
    function createERC721(string memory name, string memory symbol) public {
        ...
        _registerToken(address(newToken), TokenType.ERC721);
    }

    function createERC1155(string memory uri) public {
        ERC1155Token newToken = new ERC1155Token(uri);
        _registerToken(address(newToken), TokenType.ERC1155);
    }
     ...
     */
    function _registerToken(address tokenAddress, TokenType tokenType) internal {
        uint256 tokenId = tokenIdCounter;
        tokenIdToAddress[tokenId] = tokenAddress;
        tokenIdToType[tokenId] = tokenType; // Store the token type
        userDeployedTokens[msg.sender][tokenType].push(tokenAddress);

        emit TokenDeployed(tokenId, tokenAddress, tokenType, msg.sender);

        // Increment the token ID counter for the next deployment
        tokenIdCounter++;
    }
        ...
        returns (address tokenAddress, TokenType tokenType)
    {
        ...
        tokenType = tokenIdToType[tokenId]; // Retrieve the token type from the mapping
    }
}
